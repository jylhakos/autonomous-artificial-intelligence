...
Because quantization is lossy, the refiner re-scores candidates returned by the compressed index using original full-precision vectors. At query time, the system retrieves $\text{topK} \times \text{expansion}\_\text{rate}$ candidates from the quantized index, then the refiner recomputes exact distances on those candidates and returns the final $\text{topK}$ results. This hybrid approach maintains high recall without paying the full cost of exact search over the entire dataset.
